...
export default function BookAppointmentPage() {
  ...
  useEffect(() => {
    if (selectedDept) {
      const fetchDoctors = async () => {
        try {
          setLoading(true);
          const data: any = await appointmentAPI.getDoctors(selectedDept);
          const normalizedDoctors = (data?.doctors ?? data ?? []).map((doc: any) => ({
            id: Number(doc.id),
            name: doc.name || doc.doctor_name,
  ...
  useEffect(() => {
    if (selectedDoctor !== null && selectedDate) {
      const fetchSlots = async () => {
        try {
          setLoading(true);
          const data: any = await appointmentAPI.getSlots(Number(selectedDoctor), selectedDate);
          console.log("Slots API raw response:", data);
          console.log("SelectedDoctor:", selectedDoctor, "SelectedDate:", selectedDate);

          const normalizedSlots = (Array.isArray(data) ? data : (data?.slots || [])).map((slot: any) => ({
            id: Number(slot.id),
            time: slot.time || `${slot.start_time} - ${slot.end_time}`,
            available: slot.available ?? !slot.is_booked,
          }));
                  ...
                  <div className="space-y-3">
                    {doctors.map((doctor) => (
                      <button
                        key={doctor.id}
                        onClick={() => {
                          setSelectedDoctor(Number(doctor.id));
                          setStep(3);
                        }}
